...
| metallb-operator (metallb/metallb-operator) | BUG: manifest-install deadlock — controller/speaker mount certs secrets whose producing monitor Services only render with DEPLOY_SERVICEMONITORS=true (found coex-2) | **DRAFT ready to file: docs/metallb-operator-servicemonitors-bug-draft.md** |
| metallb-operator (metallb/metallb-operator) | BUG: webhook server's METALLB_BGP_TYPE is a static manifest value, never reconciled from the MetalLB CR's bgpBackend — validation rejects IPv6 resources in frr-k8s mode as "native bgp mode does not support ipv6" (found coex-3) | **DRAFT ready to file: docs/metallb-operator-webhook-bgptype-bug-draft.md** |
