...
1. Review-chase (updated 2026-08-20): MCO #6326 (auto-`/hold` after 3
   retest rounds — needs `/hold cancel` + re-lgtm + approve; all real CI
   green, remaining failures are the known flakes), CNO #3047 (rebased
   2026-08-17, no changes — cybertron pinged for re-lgtm, then approve),
   CNO #3046 (jcaamano, idle since Jul 15), kube-vip#15 (approved+verified,
   needs lgtm), dev-scripts#1945 (needs reviewer).
   DONE: api#2923, release#81957, runtimecfg#395,
   **installer#10718 MERGED 2026-08-17** (OPNET-781 complete).
2. Post the drafted replies on the 8 open EP threads (see session notes;
   ...
   items into the EP.
3. OBSOLETE (2026-08-17): the second downstream openshift/kube-vip PR is
   no longer needed — the rebase-bot sync #12 brought downstream main past
...
1. Nightly with the gate: install a DevPreviewNoUpgrade cluster from a real
   ...
   harness. **UNBLOCKED 2026-08-20**: installer#10718 merged + kube-vip in
   the nightly payload; still needs MCO#6326 + CNO#3047 merged for the full
   path (and kube-vip#15 + next ART build for dual-stack).
...
| Where | What | Artifact |
|-------|------|----------|
| kube-vip | **MERGED upstream**: kube-vip/kube-vip#1627 (manager + backend honor the configured kubeconfig). NOTE: the RT-mode HTTP health check (51e05fd) is ALREADY upstream as kube-vip/kube-vip#1604 (fcd3eec) — drop that patch from the downstream fork on the next rebase. The 2 route re-assertion commits (6d51cbd level-triggered, 7d27248 realm toggle) went to downstream **openshift/kube-vip#6 — CLOSED unmerged (decision resolved: the FRR table-scoped-cleanup fix alone is enough, proven run18)**; upstream re-assert **MERGED: kube-vip/kube-vip#1636**. Downstream fork synced to upstream via rebase-bot #12 (merged 2026-08-17); release-5.0 sync #15 open. STILL TO OPEN downstream: second openshift/kube-vip PR with 51e05fd (HTTP health check, cherry-pick of upstream #1604 — missing from downstream main, based before its merge) + the kubeconfig commits 1731730/8cd17f7 |
...
| metallb-operator (metallb/metallb-operator) | BUG: manifest-install deadlock — controller/speaker mount certs secrets whose producing monitor Services only render with DEPLOY_SERVICEMONITORS=true (found coex-2) | **FILED: OCPBUGS-105394** (2026-08-07, by Mat; draft: docs/metallb-operator-servicemonitors-bug-draft.md) |
| metallb-operator (metallb/metallb-operator) | BUG: webhook server's METALLB_BGP_TYPE is a static manifest value, never reconciled from the MetalLB CR's bgpBackend — validation rejects IPv6 resources in frr-k8s mode as "native bgp mode does not support ipv6" (found coex-3) | **FILED: OCPBUGS-105395** (2026-08-07, by Mat; draft: docs/metallb-operator-webhook-bgptype-bug-draft.md) |
| frr-k8s (metallb/frr-k8s) | BUG: validation webhook blanks passwordSecret before merge validation (resetSecrets) — secret-ref + inline password for a shared neighbor is permanently rejected even when values match; wedges ALL FRRConfiguration writes (found api-2; MetalLB always inlines, so any passwordSecret adopter collides) | **FILED: https://github.com/metallb/frr-k8s/issues/484** (2026-08-10, by Mat; draft: docs/frr-k8s-mixed-password-merge-bug-draft.md, PR offer included) |
...
5. RESOLVED (2026-08-13, installer#10718 review round): `port` wired end to
   end (runtimecfg FRRPeer field + both FRR template sets + CNO
   FRRConfiguration mapping); timers converted to a bare-seconds peer-data
   contract (installer converts durations, FRR templates render directly,
   CNO converts back to metav1.Duration); e2e now sets every optional peer
   field (dev-scripts#1945) and the verify step asserts negotiated
   timers/BFD at the ToR (release#82912, merged).
...
### E1. Combined lane: BGP VIP + OVN-K route advertisements — MERGED (2026-08-13)

**MERGED: https://github.com/openshift/release/pull/82912** (2026-08-13; Jira: OPNET-803) (branch
...
- **Dual-stack conversion (2026-08-06, bd39688)**: all three coexistence
  jobs at IP_STACK=v4v6 with family-aware verify steps; base lane stays
  v4. Live-validated (ledger coex-3, three producers on the dualstack
  cluster). Prereqs for green: dev-scripts#1945 (v6 ToR peer),
  MCO#6326 secondary kube-vip instances, kube-vip#1671 + RT-mode NODAD
  fix (branch rt-mode-nodad, upstream PR to open).
- **FRR runtime-state verify (2026-08-13, da8f92f4196, merged with the PR)**:
  new verify step asserts every ToR BGP session Established with negotiated
  timers 90/30 and BFD Up — closes the fail-soft gap that hid the FRR
  timer-format bug on installer#10718 (FRR logs-and-skips an invalid config
  line; sessions still establish on defaults, nothing went red). Pairs with
  dev-scripts#1945 setting every optional peer field.
...
| OPNET-783 | cluster-network-operator | #3046 (statusmanager fix — prerequisite for compact/SNO, still needs jcaamano) + **#3047 (main BGP series; CodeRabbit round of 2026-07-23 fully addressed — communities now rendered with validation, exporter limits + IPv6 [::] bind, cp-metrics runAsNonRoot; hostOverrides/passwordSecret documented as DevPreview limitations; reviewDecision APPROVED by bot, awaiting jcaamano; typed-access swap DONE 2026-07-30; e6c26833a 2026-08-13: peer `port` mapping + bare-seconds timer contract — validates whole seconds 0-65535 + pair, renders FRRConfiguration timers as metav1.Duration; rebased 2026-08-17 onto master with zero changes [range-diff-clean], awaiting cybertron re-lgtm + approve)**. Vendor bump **#3089 MERGED** (2026-07-30; needed maintainer /override of four required jobs — e2e-aws-ovn-fdp-qe + the three ipsec jobs — which the cross-PR history showed 16/16 failing on every CNO PR for 3+ days, repo-wide breakage unrelated to the bump). **NEXT: rebase #3047 + typed-access swap now unblocked.** Upstream fix consumed: **#3070 MERGED** (OCPBUGS-99074, frr-k8s CRD asn int32/int64 on 1.36 apiservers; found in run23, our duplicate #3080 closed) — **drop the DEMO-CARRY from the dev branch on next rebase** |
| OPNET-785 | baremetal-runtimecfg | **MERGED: openshift/baremetal-runtimecfg#395** (2026-08-13) — final commit 38a7b106 added the FRRPeer `port` field and documented the bare-seconds timer contract (installer#10718 review findings) |
| OPNET-787 | dev-scripts | **MERGED: openshift-metal3/dev-scripts#1929 + #1939** (ToR + BGP_VIP_MANAGEMENT knob, #1939 merged 2026-07-31). **PR OPEN: #1945** (dual-stack v6 ToR peer; extended 2026-08-13 with 67c4c6e: every optional peer field — port/timers/password/BFD/multihop — set in e2e deployments + matching ToR config, closing the coverage gap that hid the timer-format and port dead-code bugs) |
| OPNET-784 | kubevip | upstream: #1627 MERGED, #1636 MERGED, backend health-check kubernetes_addr override **MERGED: kube-vip/kube-vip#1671** (2026-08-08 — run28 finding 2/D13 fix); RT-mode DAD skip as explicit option vip_skipdad/SkipDAD, default false, both address families (IPv6 DAD blackholes second+ ECMP advertiser, found coex-3; v4 future-proofs RHEL10 conflict detection) **MERGED: kube-vip/kube-vip#1675** (2026-08-08 — DAD policy on the network object, all add paths incl. Service LB VIPs); MCO sets vip_skipdad=true on all kube-vip pods (963e35e1d); downstream **openshift/kube-vip#6 CLOSED unmerged** (re-assertion not needed — FRR fix suffices, run18); downstream fork fully synced: **#12 MERGED 2026-08-17** (rebase-bot, main past upstream #1671/#1675 — obsoletes the planned second downstream PR), **#15 OPEN** (release-5.0 sync, approved+verified, needs lgtm; the 2026-08-20 payload image f60a498 predates it, so the dual-stack fixes reach the payload only after #15 + next ART build) |
| OPNET-779 | kubevip onboarding | ART Jira FILED: **ART-21663**; CI promotion **#81957 MERGED** (2026-07-30, re-lgtm + rehearsals-ack after the conflict fix) — kube-vip CI image build + ocp/5.0 promotion live; unblocks the private-org sync/branching and, downstream, the ART payload tag; ocp-build-data **MERGED: openshift-eng/ocp-build-data#11838** (2026-08-14, + 5.1 cherry-pick #12319). **kube-vip IS IN THE PAYLOAD since 2026-08-20** (nightly 5.0.0-0.nightly-2026-08-20-024947 + ci 5.0.0-0.ci-2026-08-20-043044): three green Konflux builds initially never reached art-latest — the missing piece was the io.openshift.release.operator=true label on the image (kube-vip#17/f60a498). OPNET-779 payload track COMPLETE; see docs/kube-vip-art-onboarding.md |
