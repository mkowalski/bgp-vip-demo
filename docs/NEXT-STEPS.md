...
1. Structured BGP API per EP — **IMPLEMENTED** (2026-08-07): dedicated
   ...
   **api PR OPEN (draft): openshift/api#2972** (single squashed commit 60218f909, api-approved URL filled, suite 86/86; REVISED 2026-08-10: peer auth is passwordSecret-only — openshift-config basic-auth Secret reference mirroring frr-k8s, no inline password). CONSUMER REWORK DONE (2026-08-10: installer 1864ea3844, MCO 56ab76d64, CNO 2566dc602; live-proven except the frr-k8s same-neighbor mixed-password merge bug — ledger api-2; design fork pending user). Original plan was: installer generates the Secret manifest + reference (bootstrap keeps plaintext from install-config), MCO resolves the Secret into the internal JSON (+ secret informer, degrade on missing), CNO maps to frr-k8s neighbor passwordSecret (copy openshift-config -> openshift-frr-k8s) - consumers currently implement the inline revision and vendor 15e15a151. Consumer pushes/PRs pending user approval; consumers re-vendor + drop fork replaces once #2972 merges.** passwordSecretRef
...
| OPNET-784 | kubevip | upstream: #1627 MERGED, #1636 MERGED, backend health-check kubernetes_addr override **PR OPEN: kube-vip/kube-vip#1671** (branch backend-kubernetes-addr, 83f03d4 — run28 finding 2/D13 fix, opened by Mat 2026-08-06); RT-mode DAD skip as explicit option vip_skipdad/SkipDAD, default false, both address families (IPv6 DAD blackholes second+ ECMP advertiser, found coex-3; v4 future-proofs RHEL10 conflict detection) **PR OPEN: kube-vip/kube-vip#1675** (opened by Mat 2026-08-07; branch rt-mode-nodad, 133bc25 — DAD policy on the network object, all add paths incl. Service LB VIPs; RT-mode DADFAILED reactive recovery discussed as complement, not yet implemented); MCO sets vip_skipdad=true on all kube-vip pods (963e35e1d); downstream **openshift/kube-vip#6** (route re-assertion); second downstream PR (health check + kubeconfig) still to open |
