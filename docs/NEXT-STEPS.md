...
| metallb-operator (metallb/metallb-operator) | BUG: manifest-install deadlock — controller/speaker mount certs secrets whose producing monitor Services only render with DEPLOY_SERVICEMONITORS=true (found coex-2); related: webhook METALLB_BGP_TYPE=native hardcode rejects v6 pools in frr-k8s mode (found coex-3) | **DRAFT ready to file: docs/metallb-operator-servicemonitors-bug-draft.md** |
