...
| OPNET-784 | kubevip | upstream: #1627 MERGED, #1636 MERGED, backend health-check kubernetes_addr override **PR OPEN: kube-vip/kube-vip#1671** (branch backend-kubernetes-addr, 83f03d4 — run28 finding 2/D13 fix, opened by Mat 2026-08-06); RT-mode IPv6 NODAD fix (DAD blackholes second+ ECMP advertiser, found coex-3) **TO OPEN upstream** (branch rt-mode-nodad on mkowalski/kube-vip, 580d7d2); downstream **openshift/kube-vip#6** (route re-assertion); second downstream PR (health check + kubeconfig) still to open |
